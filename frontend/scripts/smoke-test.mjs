const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4210';
...
const adminCookie = await login('admin@dev-ai-platform.local', 'admin123');
const managerCookie = await login('manager@dev-ai-platform.local', 'manager123');
const analystCookie = await login('analyst@dev-ai-platform.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/projects', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/projects', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/projects/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/projects/approve`, {
  method: 'POST',
  headers: {
